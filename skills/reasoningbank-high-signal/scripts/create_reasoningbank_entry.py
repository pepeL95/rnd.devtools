#!/usr/bin/env python3
"""Create a reasoningbank entry scaffold with enforced naming.

Output path format: .deliverables/reasoningbank/mm-dd-yyyy-{short-name}.md
"""
...
def build_template(title: str, date_str: str) -> str:
    return f"""# {title}
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Output directory for reasoningbank entries",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    out_path = out_dir / f"{date_str}-{short_name}.md"
    if out_path.exists() and not args.overwrite:
        raise SystemExit(f"Refusing to overwrite existing file: {out_path}")

    title = short_name.replace("-", " ").title()
    out_path.write_text(build_template(title, date_str), encoding="utf-8")
    print(out_path)
    return 0
